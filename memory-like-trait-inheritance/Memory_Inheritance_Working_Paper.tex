...
\usepackage{url}
\usepackage{xurl}  % allows URLs/DOIs to break cleanly across lines
...
\url{https://archive.org/details/languagemind00chom}\\
...
\textit{Summary:} Foundational statement of the Minimalist Program; the core structure-building operation is Merge (binary set-formation), taken to be the central computational property of the faculty of language.\\
\textit{Causal role:} Supplies the explicit minimalist architecture whose innate core (Merge) later interfaces with the possibility of experience-dependent epigenetic tuning.

\subsection*{2009}
\textbf{Piattelli-Palmarini, M., Uriagereka, J., \& Salaburu, P.} (eds.) (2009). \textit{Of Minds and Language: A Dialogue with Noam Chomsky in the Basque Country}. Oxford: Oxford University Press.  
\url{https://global.oup.com/academic/product/of-minds-and-language-9780199544660}\\
\textit{Summary:} Biolinguistic volume centered on Chomsky; includes sustained discussion of Merge (notably Boeckx on its consequences for language, mind, and biology) and the minimalist conception of the faculty of language.\\
\textit{Causal role:} Bridges classical generative linguistics and contemporary biolinguistics, keeping Merge and the minimalist core visible in a biological frame.

\subsection*{1998}
\textbf{Clayton, N.~S., \& Dickinson, A.} (1998). Episodic-like memory during cache recovery by scrub jays. \textit{Nature}, 395, 272--274.  
\doi:\url{10.1038/26216}\\
...
\subsection*{2012}
\textbf{Biergans, S.~D., Jones, J.~C., Treiber, N., Galizia, C.~G., \& Szyszka, P.} (2012). DNA methylation mediates the discriminatory power of associative long-term memory in honeybees. \textit{PLoS ONE}, 7(6), e39349.  
\doi:\url{10.1371/journal.pone.0039349}\\
\textit{Summary:} DNA methyltransferases are required for the odor-specificity (discriminatory power) of associative long-term memory in honeybees, but not for memory strength itself.\\
\textit{Causal role:} Supplies direct insect evidence that DNA methylation shapes the specificity of sensory associative memory.
...
\doi:\url{10.1016/j.tins.2014.12.003}\\
...
\doi:\url{10.7551/mitpress/9780262034241.001.0001}\\
\textit{Summary:} Within the Minimalist Program (Chomsky 1995; see also Piattelli-Palmarini et~al.\ 2009), argues that the core computational operation Merge is recent, discrete, and largely uniform across the species.\\
\textit{Causal role:} Frames the evolutionary and architectural puzzle of how a largely innate, Merge-based faculty of language can still be refined by experience-dependent epigenetic channels.
...
\doi:\url{10.1038/s41576-021-00438-5}\\
...
\url{https://www.mdpi.com/journal/insects}\\
...
\doi:\url{10.1038/s41467-023-39804-8}\\
...
The literature forms a coherent causal chain. Chomsky's early work and the Minimalist Program supply the premise that core cognitive architecture is biological and innate, with Merge as the central computational operation. Clayton \& Dickinson show that non-human animals already form structured experiential memories. Insect studies (Herb 2012; Biergans et al.\ 2012) demonstrate that behaviorally relevant epigenetic states are plastic and reversible, and that DNA methylation shapes the specificity of sensory associative memory. Dias \& Ressler (2014) supply the decisive mammalian experiment: a specific parental sensory experience leaves germline DNA-methylation marks (CpG hypomethylation at Olfr151) that produce corresponding behavioral and neural sensitivities in na\"ive offspring and grand-offspring. Dias et~al.\ (2015) and Fitz-James \& Cavalli (2022) articulate the molecular routes---DNA methylation, histones, ncRNAs---that allow such marks to survive germline reprogramming. 2023 papers (Deshe in \textit{C.\ elegans}; Yang-related olfactory work) broaden the phylogenetic and mechanistic base.
...
\begin{tikzpicture}[
  ...
  empir/.style={
    ...
    minimum width=5.1cm, minimum height=0.95cm, inner sep=2.5pt
  },
  thesis/.style={
  ...
  sg/.style={
    ...
    minimum width=3.5cm, inner sep=3.5pt
  },
  arr/.style={-Latex, accent, line width=0.85pt}
]

% Empirical / literature nodes — each names the paper(s)
\node[empir] (c1) {\textbf{C1\,·\,Innate architecture}\\Chomsky 1957--1995 (Merge);\\Piattelli-Palmarini et al.\ 2009;\\Berwick \& Chomsky 2016};
\node[empir, below=of c1] (c2) {\textbf{C2\,·\,Episodic-like memory}\\Clayton \& Dickinson 1998};
\node[empir, below=of c2] (c3) {\textbf{C3\,·\,Behavioral methylation}\\Herb et al.\ 2012\\Biergans et al.\ 2012};
\node[empir, below=of c3] (c4) {\textbf{C4\,·\,Germline transmission}\\Dias \& Ressler 2014};
\node[empir, below=of c4] (c5) {\textbf{C5\,·\,Molecular mechanisms}\\Dias et al.\ 2015\\Fitz-James \& Cavalli 2022};
\node[empir, below=of c5] (c6) {\textbf{C6\,·\,Phylogenetic breadth}\\Deshe et al.\ 2023\\(+ 2023 OR / eLife work)};
\node[thesis, below=of c6] (th) {\textbf{THESIS}\\Experiences epigenetically pass\\behavioral tendencies to offspring};

% Causal flow we are proposing
\foreach \a/\b in {c1/c2, c2/c3, c3/c4, c4/c5, c5/c6, c6/th}
  \draw[arr] (\a) -- (\b);

% Stop-gap bridges (explicit, not obstacles)
\node[sg, right=1.15cm of c1] (sg1) {\textbf{\color{orange}SG1 · Interface}\\Minimalist innate core\\(Merge-based) can still be\\tuned by experience-dependent\\epigenetic marks};
\node[sg, right=1.15cm of c3] (sg2) {\textbf{\color{orange}SG2 · Homology}\\Mouse / insect / nematode\\sensitivities $\to$ human\\behavioral tendencies};
\node[sg, right=1.15cm of c5] (sg3) {\textbf{\color{orange}SG3 · Content vs bias}\\What transmits is adaptive\\sensitivity, not the full\\autobiographical episode};

% Legend
\node[below=0.55cm of th, font=\tiny, align=left, text=muted] {
